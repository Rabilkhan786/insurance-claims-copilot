...
class PolicyDataStore:
    ...
    def add_room_rent(
        ...
        policy_uin: str,
        insurer: str,
        sum_insured_min: float,
        sum_insured_max: float,
        ...
        with self._connect() as connection:
            connection.execute(
                "INSERT INTO policy_room_rent "
                "(policy_uin, insurer, sum_insured_min, sum_insured_max, "
                " room_rent_limit, icu_limit, page) VALUES (?, ?, ?, ?, ?, ?, ?)",
                (
                    policy_uin,
                    insurer,
                    sum_insured_min,
    ...
    def find_room_rent(self, policy_uin: str, sum_insured: float) -> dict | None:
        """Find the room rent band that a given sum insured falls into."""
        with self._connect() as connection:
            row = connection.execute(
                "SELECT * FROM policy_room_rent WHERE policy_uin = ? "
                "AND (sum_insured_min IS NULL OR sum_insured_min <= ?) "
                "AND (sum_insured_max IS NULL OR sum_insured_max >= ?)",
                (policy_uin, sum_insured, sum_insured),
            ).fetchone()
        return dict(row) if row else None
    ...
    def add_hospital(
        ...
        cashless_status: str = "available",
    ) -> None:
        with self._connect() as connection:
            connection.execute(
                "INSERT INTO network_hospitals "
                "(hospital_name, city, insurer, cashless_status) "
                "VALUES (?, ?, ?, ?)",
                (hospital_name, city, insurer, cashless_status),
            )

    def find_hospital(self, hospital_name: str, insurer: str | None = None) -> dict | None:
        query = "SELECT * FROM network_hospitals WHERE LOWER(hospital_name) LIKE ?"
        params: list = [f"%{hospital_name.lower()}%"]
        if insurer:
            query += " AND LOWER(insurer) = ?"
            params.append(insurer.lower())
    ...
    def add_day_care_procedure(
        ...
        with self._connect() as connection:
            connection.execute(
                "INSERT INTO day_care_procedures "
                "(procedure_name, procedure_category) VALUES (?, ?)",
                (procedure_name, procedure_category),
            )

    def is_day_care_procedure(self, procedure_name: str) -> bool:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT 1 FROM day_care_procedures "
                "WHERE LOWER(procedure_name) LIKE ?",
                (f"%{procedure_name.lower()}%",),
            ).fetchone()
