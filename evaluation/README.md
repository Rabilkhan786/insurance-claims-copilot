...
One model does both jobs -- Groq's `gpt-oss-120b`, on the same `GROQ_API_KEY`
the live app already uses. Scores are written to `baseline_results.json`,
which is not committed: the numbers depend on live retrieval and a live model
call, so a stale copy would describe a run that may no longer match the code.
Run it yourself to get current numbers.
...
**The judge marks its own homework.** One model, Groq's `gpt-oss-120b`,
generates the answers and then grades them. Self-grading bias inflates
faithfulness and relevancy. Treat the numbers as directional, not absolute.

**Groq's rate limit can turn into missing scores.** `nan` means a judge call
did not complete, not that the answer scored zero -- it is excluded from the
mean, so a metric may be averaged over fewer questions than were run. The
per-question lines printed during the run show which. This is the actual
tradeoff of using one provider for both roles: it is simpler to configure,
at the cost of both jobs competing for the same rate-limit budget.
